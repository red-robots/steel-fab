<?php
/**
 * Template Name: Engineering & Detailing
...
<div id="primary" class="content-area-full content-default page-values-template single-services-page engineering <?php echo $has_banner ?>">
	<main id="main" class="site-main" role="main">
		...
		<?php while ( have_posts() ) : the_post(); ?>
			...
			if($text_blocks) { ?>
			<!--- Alternate Text and Image blocks -->
			...
			$has_row3_columns = ( $row3_column_content ) ? 'has-columns decorative-columns':'no-columns';
			$has_row3_columns .= ( $row3_column_content ) ? ' columns-' . count($row3_column_content) : '';
			if( $row3_title || $row3_text || $row3_column_content ) { ?>
			<div id="row3" class="section-wrap row3-services <?php echo $has_row3_columns ?>">
			...
			<div class="yellow-bar yellow-section">
				<div class="wrapper narrow text-center">
					<?php if ($row4_title) { ?>
					<h2 class="col-title"><?php echo $row4_title ?></h2>	
					<?php } ?>
					<?php if ($row4_text) { ?>
					<div class="col-text"><?php echo email_obfuscator($row4_text) ?></div>	
					<?php } ?>
				</div>
			</div>
			<?php } ?>
			...
			$row5_bg_img = get_field("row5_bg_img");
			$row5BgImg = ($row5_bg_img) ? ' style="background-image:url('.$row5_bg_img['url'].')"':'';
			if ($row5_title || $row5_text) { ?>
			<div id="row5" class="section-wrap dark-blue-section <?php echo $has_row3_columns ?>">
				<?php if ($row5BgImg) { ?>
				<div class="imagebg"><div <?php echo $row5BgImg ?>></div></div>
				<?php } ?>
				<div class="wrapper narrow text-center">
					<?php if ($row5_title) { ?>
					...
					<?php } ?>
					<?php if ($row5_text) { ?>
					<div class="col-text"><?php echo email_obfuscator($row5_text) ?></div>	
					<?php } ?>
				</div>
			</div>
			<?php } ?>
			...
			<div id="row6" class="section-wrap mid-content-area">
				<div class="wrapper text-center">
					<div class="column-content">
						<?php if ($row6_title) { ?>
						<h3 class="t1"><?php echo $row6_title ?></h3>	
						<?php } ?>
						<?php if ($row6_text) { ?>
						<div class="t2"><?php echo email_obfuscator($row6_text) ?></div>	
						<?php } ?>
					</div>
				</div>
			</div>
